...
module.exports = class PSpectrum {
  constructor() {
    ...
    fs.readdirSync(this._tempDir).forEach((file) =>
      fs.unlinkSync(join(this._tempDir, file))
    );
  ...
  spawn() {
    ...
    this.child = spawn(join(this._tempDir, "PSpectrum.exe"), [], {
      cwd: this._tempDir,
    });
  ...
  _normalize(data) {
    ...
    data.forEach((d, i) =>
      d >= this._normalize_buffer[i] * 7 ? (fails += 1) : 0
    ); // increase if the signal of a channel multiplys itself by 7 from the previous step
    if (fails > 200 && fails < data.length) data = this._normalize_buffer;

    // simple normalization
    this._normalize_buffer.forEach(
      (_, i) => (data[i] = (data[i] + this._normalize_buffer[i]) / 2)
    );
