module.exports = class Render {
  constructor(plugin) {
    ...
    let possibleElements = document.querySelectorAll(`div.container-YkUktl`);
    ...
    this._mupdate = setInterval(() => {
      ...
      // center frequency
      this._center = plugin?.settings?.get("visualizerCenter", false) ?? false;
    ...
    (async () => {
      ...
      this.getCurrentStatus = () => document.querySelector(`div.avatar-1EWyVD.wrapper-1VLyxH rect`).getAttribute("mask").replace("url(#svg-mask-status-","").replace(")","");
    })();
  }
   ...
   */
  drawNewFrame(data = []) {
    let padding = this.plugin?.settings?.get("padding", 1) ?? 1;
    ...
    let multiplyer = this.mult * 50;
    let width = half / (data.length / 2) - padding;

    this._ctx.fillStyle = this.color;

    // reverse channels if center is disabled
    if (!this._center) {
      left.reverse();
      right.reverse();
    }
    ...
    left.reverse();
    left.forEach((d, i) =>
      this._ctx.fillRect((i-1) * (width + padding), height, width, -(d * multiplyer))
    );

    // draw right channel
    right.forEach((d, i) =>
      this._ctx.fillRect(i * (width + padding) + half, height, width, -(d * multiplyer))
    );
  }
  ...
  prepareData(data) {
    ...
    //data = data.filter((_, i) => i % 3 === 0); //FIXME: somehow this breaks the visualizer
