...
module.exports = class Render {
  constructor(plugin) {
    ...
    this._canvas.setAttribute("style", `position:absolute; left: 0; bottom: 0; width: ${this._parent.clientWidth}px; height: ${this._parent.clientHeight};`);
    ...
    this._mupdate = setInterval(() => {
      ...
      if (plugin?.settings?.get("manualTheming", false) ?? false) this.color = this.getManualColor();
    ...
    (async () => {
      ...
      this.getCurrentStatus = () => getState().statuses[getCurrentUser().id] ?? "online";
    })();
  }
  ...
  drawNewFrame(data = []) {
    ...
    // draw left channel
    left.reverse();
    left.forEach((d, i) => this._ctx.fillRect(1 * (i-1), height, 1, -(d * multiplyer)));
  ...
  getManualColor() {
    ...
    switch (status) {
      case "online":
        return "#" + (this.plugin?.settings?.get("color-online", "1ed860") ?? fallback)

      case "idle":
        return "#" + (this.plugin?.settings?.get("color-idle", "d8d51e") ?? fallback)

      case "dnd":
        return "#" + (this.plugin?.settings?.get("color-dnd", "d81e1e") ?? fallback);

      case "invisible":
        return "#" + (this.plugin?.settings?.get("color-invisible", "4f4f4f") ?? fallback);
  ...
  getThemeColor() {
    ...
    switch (status) {
      case "online":
        result = getComputedStyle(document.querySelector('#pspectrum-visualizer')).getPropertyValue("--pspectrum-online-color");
        break;

      case "idle":
        result = getComputedStyle(document.querySelector('#pspectrum-visualizer')).getPropertyValue("--pspectrum-idle-color");
        break;

      case "dnd":
        result = getComputedStyle(document.querySelector('#pspectrum-visualizer')).getPropertyValue("--pspectrum-dnd-color");
        break;

      case "invisible":
        result = getComputedStyle(document.querySelector('#pspectrum-visualizer')).getPropertyValue("--pspectrum-invisible-color");
        break;
    ...
    if (result == "") {
      result = getComputedStyle(document.querySelector('#pspectrum-visualizer')).getPropertyValue("--pspectrum-color");
      result = result != "" ? result : getComputedStyle(this._canvas).getPropertyValue("--spotify-color");
    }
